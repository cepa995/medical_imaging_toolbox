...
DICOM_PATH = 'dicom_dir_path'
SUBJECT_ID = 'subject_id'


class DicomTagProcessorError(Exception):
    """ Raised when DicomTagProcessor class could not be initialized"""
    pass
...
class DicomTagProcessor:
    ...
    def __init__(self, tags_of_interest):
        if tags_of_interest:
            raise DicomTagProcessorError("Specify list of relevant DICOM tags.")
    ...
    @staticmethod
    def _get_dicom_df(subject_id, dataset_path):
        """ Returns pandas.DataFrame object for specific DICOM file
        :param dataset_path: dicom directory path
        :return: df of columns: subject_id, sequence_name, path
        """
        first_dcm_file = get_path_to_first_dicom_file(dataset_path)
        return pd.DataFrame({
            SUBJECT_ID: subject_id,
    ...
    def get_tags_of_interest(self, subject_id, dicom_file):
        keys, subject_list = self.tags_of_interest + [SUBJECT_ID], []
        for _, row in DicomTagProcessor._get_dicom_df(subject_id, dicom_file).iterrows():
...
class DicomReader:
    ...
    def __init__(self, dcm_root: str):
        ...
        self.itk_image = DicomReader.read_from_dicom_series(dcm_root)
